// Globe A — netActiv style network sphere. Adapted from /home-globe.js,
// stripped of the glitch/overcaffeinated easter egg (not relevant here)
// and turned into a mountable controller so app.js can swap it in/out.
window.GlobeA = (function () {
  function create(canvas) {
    const ctx = canvas.getContext('2d');

    let W, H, cx, cy, R, Rdraw = 0, rafId = null;
    ...
    function buildScene() {
      const nodeCount   = Math.max(8,  Math.round(44 * COUNT_MULT));
      const edgeCount   = Math.max(5,  Math.round(50 * COUNT_MULT));
      const packetCount = Math.max(3,  Math.round(28 * COUNT_MULT));
      ...
      hubIdxs.forEach(h => {
        ...
        while (got < target && t++ < 600) {
          ...
          if (tooParallel(ax, edgeAxes, MAX_PARALLEL)) continue;
      ...
      packets = Array.from({ length: packetCount }, () => {
        ...
        if (rawSpeed < SLOW_FLOOR_RAW && (slowCount >= SLOW_CAP || slowEdges.has(edge))) {
          speed = SLOW_FLOOR_RAW + Math.random() * (0.0032 - SLOW_FLOOR_RAW);
    ...
    function render(ts) {
      ...
      nodes.forEach(nd => {
        ...
        const a     = Math.min(0.98, vis * (0.55 + 0.45 * pulse));
